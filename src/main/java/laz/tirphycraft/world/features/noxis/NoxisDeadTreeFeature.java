...
public class NoxisDeadTreeFeature extends Feature<NoFeatureConfig> {
	...
	public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> generator, Random rand,
		...
		if (worldIn.getBiome(pos) != TirphycraftBiomes.N_THORNS.get()) return false;
		...
		for (int i = pos.getY() + size; i > 0; i--) {
			...
			if (worldIn.getBlockState(p).isSolid()) break;
	...
	public void makeBrench(IWorld world, BlockPos pos, Random rand, float dirx, float diry, float dirz) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (world.getBlockState(p) == Blocks.AIR.getDefaultState()) setBlockState(world, p, LOG);
			tempx += dirx;
			tempy += diry;
			tempz += dirz;
		}

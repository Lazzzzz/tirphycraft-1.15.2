...
public class FrozDungeonStructure extends Structure<NoFeatureConfig> {
	public FrozDungeonStructure(Function<Dynamic<?>, ? extends NoFeatureConfig> config) {
		super(config);
	}
	 ...
	 * 
	 * This is using vanilla's default algorithm to determine chunks that this
	 * structure can spawn in.
	 */
	@Override
	protected ChunkPos getStartPositionForPosition(ChunkGenerator<?> chunkGenerator, Random random, int x, int z,
		...
		// this means Run Down Houses cannot be closer than 7 chunks or more than 12
		// chunks
		int maxDistance = 40;
		int minDistance = 20;
	...
	public String getStructureName() {
		return Tirphycraft.MOD_ID + ":froz_dungeon";
	}
	...
	public Structure.IStartFactory getStartFactory() {
		return FrozDungeonStructure.Start::new;
	}
	...
	/*
	 * Visit the vanilla FortressStructure and NetherChunkGenerator's
	 * getPossibleCreatures method for how they spawn specific enemies in their
	 * structure over time.
	 *
	 * It requires the getSpawnList method to be defined in the structure class
	 * returning a List<Biome.SpawnListEntry> (the mob) and the chunk generator
	 * class's getPossibleCreatures method needs to be overridden to have a check
	 * for the structure.
	 *
	 * Thus, it is easier to do this with your own chunkgenerator in your own
	 * dimension instead of trying to make this work in vanilla's chunkgenerator. If
	 * you want your mobs to spawn in your structures in vanilla's chunkgenerators,
	 * you'll have to use the mob spawn event, check if the mob being spawned is
	 * gonna be in your structure, and replace the entity with the mobs you want
	 * spawning in your structure. It's a bit more work but it's the best we can do.
	 */
	// public List<Biome.SpawnListEntry> getSpawnList()
	// {
	// // Won't work right now as vanilla's OverworldChunkGenerator class has no
	// check for this structure in getPossibleCreatures method.
	// return NETHER_FORTRESS_ENEMIES; // A list of SpawnListEntries that holds all
	// the mobs this structure can spawn.
	// }
	 ...
	 * 
	 * If you want to do dimension checking, I would do it in the Init method. It
	 * will make the locate command say the structure is spawning in the blacklisted
	 * dimension but the structure won't actually spawn at all. This is much better
	 * than making the game become unresponsive completely. You can send a message
	 * from the server to the players there saying the dimension cannot spawn the
	 * structure and to ignore the false positive from the locate command.
	 * 
	 * This may be called func_225558_a_ instead of canBeGenerated on older
	 * mappings.
	 */
	@Override
	...
	public static class Start extends StructureStart {
		...
		public void init(ChunkGenerator<?> generator, TemplateManager templateManagerIn, int chunkX, int chunkZ,
			...
			FrozDungeonPiece.start(templateManagerIn, blockpos, Rotation.NONE, this.components, this.rand);
